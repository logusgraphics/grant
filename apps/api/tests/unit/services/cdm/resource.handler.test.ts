...
import { buildExternalKey } from '@/services/cdm/identity.helper';
...
describe('ResourceHandler', () => {
  ...
  it('teardown is a no-op when no CDM resources exist', async () => {
    ...
    expect(syncRepo.listCdmResourceIdsForProject).toHaveBeenCalledWith(projectId, ctx.tx);
    expect(syncRepo.bulkSoftDeleteCdmResources).not.toHaveBeenCalled();
  });
  ...
  it('teardown bulk-soft-deletes CDM resources + pivots when present', async () => {
    ...
    const ctx: CdmTeardownContext = { projectId, scope, tx: { __tx: true } };
    await handler.teardown(ctx);
    expect(syncRepo.bulkSoftDeleteCdmResources).toHaveBeenCalledWith(ids, projectId, ctx.tx);
  });
