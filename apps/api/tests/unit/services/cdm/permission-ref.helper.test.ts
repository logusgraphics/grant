...
} from '@/services/cdm/permission-ref.helper';
...
describe('resolveSinglePermissionRef', () => {
  it('returns null for permissionKey-only refs (deferred to apply time)', async () => {
    const syncRepo = { resolvePermission: vi.fn() };
    ...
    expect(out).toBeNull();
    expect(syncRepo.resolvePermission).not.toHaveBeenCalled();
  });

  it('throws ValidationError when neither permissionKey nor slug+action is provided', async () => {
    const syncRepo = { resolvePermission: vi.fn() };
    await expect(
      resolveSinglePermissionRef(syncRepo as never, { permissionId: 'abc' }, {} as never)
    ).rejects.toThrow(/must include resourceSlug \+ action, or permissionKey, or permissionId/);
  });

  it('delegates to syncRepo.resolvePermission for slug+action refs', async () => {
