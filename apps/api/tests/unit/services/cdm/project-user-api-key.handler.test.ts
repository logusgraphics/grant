...
import { ProjectUserApiKeyCdmHandler } from '@/services/cdm/project-user-api-key.handler';
...
function buildHandler(deps?: {
  ...
  return {
    handler: new ProjectUserApiKeyCdmHandler(
      syncRepo as never,
      exportRepo as never,
      apiKeys as never,
...
describe('ProjectUserApiKeyCdmHandler', () => {
  ...
  it('teardown deletes each CDM api key with hard delete', async () => {
    ...
    const { handler, apiKeys } = buildHandler({
      syncRepo: { listCdmProjectUserApiKeyIdsForProject },
      apiKeys: {
        createApiKeyForCdmImport: vi.fn(),
        deleteApiKey,
      },
