...
/**
 * Handler for the CDM `permissions` section. Owns CDM-marked permission rows
 * (`permissions.metadata.cdmImport.kind === 'permission'`) and their
 * `project_permissions` membership for the importing project.
 *
 * Export includes every permission linked to the project (catalog + CDM).
 * Catalog rows carry `metadata.cdmExportCatalogSnapshot` so apply binds to
 * existing entities instead of creating duplicates.
 *
 * Order = 4: runs after {@link ResourceHandler} (2, so `resourceKey` resolves
 * via `produced.resourceIds`) and before {@link TagHandler} (5) /
 * {@link RoleTemplateHandler} (10) / {@link UserAssignmentHandler} (20) so
 * those handlers can resolve `permissionKey` against `produced.permissionIds`.
 *
 * Teardown deletes only CDM-marked permissions belonging to this project plus
 * their pivot rows (`project_permissions`, `group_permissions`,
 * `permission_tags`). Grant's global system catalog is never touched.
 */
export class PermissionHandler implements ICdmEntityHandler<
  PermissionCdmInput,
  PermissionCdmInput
> {
  ...
  public async teardown(ctx: CdmTeardownContext): Promise<void> {
    ...
    const permissionIds = await this.syncRepo.listCdmPermissionIdsForProject(ctx.projectId, tx);
    if (permissionIds.length === 0) return;
    await this.syncRepo.bulkSoftDeleteCdmPermissions(permissionIds, ctx.projectId, tx);
  }
