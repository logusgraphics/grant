 ...
 *
 * Order = 2: runs before {@link PermissionHandler} (4), so permissions can
 ...
 */
export class ResourceHandler implements ICdmEntityHandler<ResourceCdmInput, ResourceCdmInput> {
  ...
  public async teardown(ctx: CdmTeardownContext): Promise<void> {
    ...
    const resourceIds = await this.syncRepo.listCdmResourceIdsForProject(ctx.projectId, tx);
    if (resourceIds.length === 0) return;
    await this.syncRepo.bulkSoftDeleteCdmResources(resourceIds, ctx.projectId, tx);
  }
