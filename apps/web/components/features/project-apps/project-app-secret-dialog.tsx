...
export interface ProjectAppSecretDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  clientId: string;
  clientSecret: string | null;
  /** Scope for server-side integration (tenant + id). */
  scope?: { tenant: string; id: string } | null;
}

export function ProjectAppSecretDialog({
  open,
  onOpenChange,
  clientId,
  clientSecret,
  scope,
}: ProjectAppSecretDialogProps) {
  const t = useTranslations('projectApps.secretDialog');
  ...
  const handleDownloadCredentials = () => {
    ...
    const credentials = {
      ...
      scope: scope ?? null,
      apiBaseUrl,
      authorizeUrl: `${apiBaseUrl}/api/auth/project/authorize`,
      tokenInfoUrl: `${apiBaseUrl}/api/auth/project/app-info`,
    };
    ...
    a.href = url;
    a.download = `project-app-credentials-${clientId}.json`;
    a.click();
